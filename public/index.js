...
const EVENTS = {
  SINGULARITY_OPEN: 'SingularityEvent-open',
  SINGULARITY_CLOSE: 'SingularityEvent-close',
  SINGULARITY_GET_USER_INFO: 'SingularityEvent-getConnectUserInfo',
  SINGULARITY_REQ_PERSONAL_SIG: 'SingularityEvent-requestPersonalSignature',
  SINGULARITY_LOGIN: 'SingularityEvent-login'
}

/** EXTERNAL SCRIPT Injection starts */
const createScript = (d, s, id, src, callback = () => {}) => {
...
window.onload = () => {
  ...
  function setSingularityEvents() {
    window.SingularityEvent = {
      ...
      requestPersonalSignature: async (messageToSign) => {
        if(window.Singularity.localStore.loginMode === LOGIN_MODES.WEB3) {
          singularityBlInstance.handleWeb3PersonalSign(messageToSign);
          return;
        }
  ...
  function addEventListner() {
    window.addEventListener('message', (e) => {
      try {
        ...
        const { SINGULARITY_CLOSE, SINGULARITY_OPEN, SINGULARITY_LOGIN } = EVENTS;
        if(name === SINGULARITY_CLOSE) {
          setTimeout(() => {
            document.getElementById('SingularityEvent__wrapper__iframe').style.display='none';
            singularityBlInstance.updateLocalStore({event: SINGULARITY_CLOSE, state: false});
          }, 100);
        }
        if(name === SINGULARITY_OPEN) {
          document.getElementById('SingularityEvent__wrapper__iframe').style.display='block';
          singularityBlInstance.updateLocalStore({event: SINGULARITY_OPEN, state: true});
        }
        if(name === 'SingularityEvent-mounted') {
          setMountingInterval();
        }
        if(name === SINGULARITY_LOGIN) {
          const { userMetaData, token, mode } = JSON.parse((e.data));
          if(userMetaData && token) {
            localStorage.setItem(SINGULARITY_ACCOUNT_DATA, JSON.stringify({...JSON.parse(userMetaData), token}));
          }
          singularityBlInstance.updateLocalStore({ event: SINGULARITY_LOGIN, state: mode })
        }
